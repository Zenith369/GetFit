...
import 'package:pose_detection_realtime/ExcerciseListingScreen.dart';
import 'package:pose_detection_realtime/data/ExcerciseDataClass.dart';
import 'package:pose_detection_realtime/main.dart';

class ExcerciseDetectionScreen extends StatefulWidget {
  ExcerciseDetectionScreen({Key? key, required this.exercisesList})
    : super(key: key);
  ExcerciseDataClass exercisesList;
  @override
  _ExcerciseDetectionScreenState createState() =>
      _ExcerciseDetectionScreenState();
}

class _ExcerciseDetectionScreenState extends State<ExcerciseDetectionScreen> {
  dynamic controller;
  bool isBusy = false;
  late Size size;
  ...
  doPoseEstimationOnFrame() async {
    ...
    _scanResults = poses;
    if (poses.length > 0) {
      if (widget.exercisesList.type == ExerciseType.PushUps) {
        detectPushUp(poses.first.landmarks);
      } else if (widget.exercisesList.type == ExerciseType.Squats) {
        detectSquat(poses.first.landmarks);
      } else if (widget.exercisesList.type == ExerciseType.PlankDownwardDog) {
        detectPlankToDownwardDog(poses.first);
      } else if (widget.exercisesList.type == ExerciseType.JumpingJack) {
        ...
        Align(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: widget.exercisesList.color,
              borderRadius: BorderRadius.circular(50),
            ),
            child: Center(
              child: Text(
                widget.exercisesList.type == ExerciseType.PushUps
                    ? '$pushUpCount'
                    : widget.exercisesList.type == ExerciseType.Squats
                    ? '$squatCount'
                    : widget.exercisesList.type == ExerciseType.PlankDownwardDog
          ...
          child: Container(
            ...
            child: Center(
              child: Row(
                children: [
                  Text(
                    widget.exercisesList.title,
                    style: TextStyle(color: Colors.amber, fontSize: 25.0),
                  ),
                  Image.asset('assets/${widget.exercisesList.image}')
